...
// Fetch all users (excluding admins & super admins)
$sql = "SELECT u.user_id, u.first_name, u.last_name, u.email, u.user_level_id, u.is_active 
        FROM users u 
        WHERE u.user_level_id = 1"; // Only fetch members
$stmt = $db->prepare($sql);
$stmt->execute();
...
foreach ($vendor_rsvps as $rsvp) {
  $vendor_rsvp_map[$rsvp['vendor_id']][] = $rsvp;
}
...
// Organize market weeks per vendor for dropdown selection
$market_weeks_map = [];
